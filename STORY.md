...
The packaged `stdlib/Prelude.tel2` stays within this non-recursive direct fragment: finite
Bool operations, Nat predicates/conversions, addition, successor, and explicit
Nat doubling. `stdlib/LegacyPrelude.tel2` contains only six honest monomorphic or
modernized aliases. `design/PRELUDE_MIGRATION.md` accounts for all 50 historical
Prelude names; Church encodings, higher-order combinators, partial abort, and
unplaced list/dynamic recursion are not presented as migrated.
...
`T3.Compiler.ClosedRecursion` constructs closed iterate, fold, and while core
terms from directly compiled components and proves exact erasure plus value
preservation via `ε-factor`. It also covers pairwise `MergeS` of independent
boxed closed results, which composes to the environment shape used by the
implementation.
...
of the Megaparsec implementation. In particular, the closed-recursion theorem
assumes `Direct` evidence and does not verify Haskell's free-variable or
whole-entry recognition. Parser correctness, alias resolution, enum coverage,
...
general append, or modal placement. Only independent closed whole-entry
iteration, fold, and literal-capped while are accepted; dynamic bounds,
dependent/nested recursion, helper recursion, and captured loops are not. Named
