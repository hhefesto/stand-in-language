...
compileDirect UIterC         = Left (RecursionRequiresPlacement Iteration)
compileDirect UFoldC         = Left (RecursionRequiresPlacement Fold)
compileDirect (UWhileC _)    = Left (RecursionRequiresPlacement While)
...
eraseMorph IterCS         = UIterC
eraseMorph FoldCS         = UFoldC
eraseMorph (WhileCS a)    = UWhileC (stripSTy a)
