...
module Telomare.Compat.PrettyPrint where

import Control.Monad.State (State)
import Data.Map (Map)
import Telomare.Compat.Syntax (AbortableF (..), BasicExprF (..), CompiledExpr,
                               CompiledExprF (..), DataType (..), FunctionIndex,
                               HighTermF (..), LamTermF (..), LamType (..),
                               LocTag, ParserTermF (..), PartialType,
                               PartialTypeF (..), PatternF (..), StuckExpr,
                               StuckExprF (..), StuckF (..), Term1, Term3 (..),
                               Term3F (..), UnprocessedParsedTerm (..),
                               UnprocessedParsedTermF (..), b2i,
                               convertAbortMessage, forget, indentWithChildren',
