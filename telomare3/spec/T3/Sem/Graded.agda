...
--
-- The M1 consolidation (design/TELOMARE3-DESIGN.md §2.4): telomare2.agda's
-- four hand-written resource functors (⟦_⟧C work, ⟦_⟧D dup, ⟦_⟧SP space —
-- ~200 near-identical lines) collapse into one interpretation ⟦_⟧G
...
--
-- Machine-independence (§16 debt 3, charter §3): the cost object is the
-- algebra's carrier ℳ; a backend is a monotone homomorphism out of it.
-- The measured 45 ITRS/tel is the ratio of two homomorphic images and
-- exits the semantics entirely.
--
...
module Interp (R : CostAlgebra) where
  ...
  -- computes the specification's value.  (The generic half of what
  -- telomare2.agda re-established per functor by construction.)
  private
...
-- boxes free (their contents pay when run).  Matches telomare2.agda ⟦_⟧C
...
-- Dup grade: the functor the bend-port experiments proved was missing.
-- Zero on all affine code; sizeT at dupS (THE charge), 1 at dupNatS (atom
-- exemption), and — new in telomare3 — sizeT per guardS/whileS probe: a
