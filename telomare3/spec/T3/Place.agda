...
-- property (the discharge of telomare2 §16 debt 2 on the compiler-owned
-- fragment; charter §2.5).
--
-- Three layers:
--
-- 1. ERASURE ε : E → S and the FACTORIZATION THEOREM
--        stripV ∘ ⟦_⟧V ≡ ⟦ ε _ ⟧VS ∘ stripV
--    — telomare2's ⟦!A⟧T = ⟦A⟧T upgraded to a functor identity:
...
-- programs (Telomare3.Infer mirrors it); the categorical fragment below
...
-- constraints are min-closed; telomare2 §8's "inference degenerates from
-- ILP toward level assignment", now a lemma).
