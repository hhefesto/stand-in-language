------------------------------------------------------------------------
-- T3.Core.Syntax — morphisms of the telomare3 core category E.
--
-- Ported from design/telomare2.agda §3, with the M1 additions recorded in
-- design/TELOMARE3-DESIGN.md §4 (milestone M1):
--
--   * assocS/unassocS/lunitS — value-trivial affine SMC plumbing the
--     point-free examples need (the T2 backend plan flagged their absence).
--   * whileS — fuel + test iteration (design doc §7 listed it; the
--     telomare2 skeleton omitted it).  Its test probes the loop state
...
--
-- The affine discipline, verbatim from the predecessor:
-- weakening (weakS, exlS, exrS) is free; there is NO fork and NO dup on
...
--
-- boxValS soundness (the telomare2 formalization discovery, easy to get
-- wrong): promotion is EMPTY-CONTEXT only, (unit ⇨ B) → (unit ⇨ !B).
-- A general A ⇨ !A would smuggle contraction: dupS ∘ boxVal would copy an
-- unboxed open input.
--
-- DECISION (design doc §14.1, recorded): foldS keeps the pragmatic typing
