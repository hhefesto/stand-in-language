------------------------------------------------------------------------
-- T3.Abstract — the Possible-successor: budgets by calculated abstract
-- interpretation (charter §2.6, milestone M4).
--
-- Because telomare3 recursion carries its fuel as DATA, "recursion
...
--     VALIDATION S2 (an inner site is sized to the max over all outer
--     iterations — Girard's level-by-level bound) is therefore the
...
--     denotational content of "the inferred budget suffices", replacing
--     telomare1's dynamic AbortRecursion probing.
--
-- Oracle mapping (test/BudgetOracle.hs mirrors this module): our budget
-- = the semantic fuel bound; telomare1's churchK = bound + 2 (VALIDATION
-- S1, its church-tower elaboration constant — cited, not re-derived).
------------------------------------------------------------------------
...
-- additional fuel returns the same value.  This replaces telomare1's
-- dynamic AbortRecursion probing: the surface meaning "run at the
