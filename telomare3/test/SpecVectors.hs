...
-- @telomare3\/spec\/T3\/Examples\/Basics.agda@, transcribed 1:1 by name.
-- The Agda side is the ground truth (checked by @checks.telomare3-spec@);
-- a changed example breaks that check, and its twin here breaks
-- @cabal test telomare3@ — the sync convention that keeps mirror and spec
-- paired.
module SpecVectors (specVectors, isZero, predS) where

import Telomare3.Core
import Telomare3.Denotation
