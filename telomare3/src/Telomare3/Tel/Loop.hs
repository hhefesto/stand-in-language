...
-- | The telomare transcript IO loop, telomare3 edition — the protocol of
-- telomare1's funWrap\/evalLoopCore (src\/Telomare\/Eval.hs:140-244):
--
...
import Telomare (RunTimeError (AbortRunTime))
import Telomare3.Tel.Eval

-- ── encodings (telomare1 s2b/b2s over machine values) ───────────────────────
...
iterateMain mainClosure inp =
  ...
  in applyClosure mainClosure inputValue >>= \result ->
       -- telomare1 checkError: an abort embedded anywhere in the result
      ...
      in case r of
           ...
           Right (out, next) -> do
             ...
             case next of
               Nothing -> pure total
               Just ns -> isEOF >>= \case
                 True  -> pure total   -- clean EOF (telomare1 crashes here)
                 False -> do
                   line <- getLine
