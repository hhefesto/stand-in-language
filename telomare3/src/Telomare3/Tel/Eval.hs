...
-- | The telomare3 Tier-2 runtime: a metered environment machine for the
-- .tel core (the charter's "deoptimize, never reject" made real).
--
-- Reference semantics: telomare1's @AbstractRunTime CompiledExpr@ instance
-- (src\/Telomare\/Possible.hs basicStep\/stuckStep\/abortStep), mirrored
-- rule-for-rule with two deliberate deviations, both documented in
-- design\/TELOMARE3-DESIGN.md:
--
--   1. __Native recursion, no sizing.__ telomare1 bakes a church tower
--      @SetEnv^(n+1) Env@ (n from Possible.hs) into every @{t,s,b}@ site;
--      the tower builds the recursion ladder @rWrap^n(abort)@ eagerly.
--      telomare3 puts 'TUnbounded' in the same position and represents the
...
--      every program telomare1 accepts, and additionally runs the ones it
--      rejects as unsizable.
--
--   2. __Lazy gate selection.__ For the syntactic if\/then\/else shape
--      @SetEnv (Pair (Gate else then) scrutinee)@ (telomare1's @iteB_@,
...
--      selected path are identical to telomare1's; dead-branch effects
--      (aborts telomare1 computes and discards) simply never happen.
--
-- Aborts are VALUES ('VAborted'), exactly as in telomare1: they propagate
...
-- iteration result is an error ('findAbort', telomare1's @checkError@).
-- Payload truncation matches @abortStep@ (Zero\/Pair structural,
-- everything else Zero).
module Telomare3.Tel.Eval
  ( TelExpr (..)
  , Value (..)
  , Meter (..)
  , emptyMeter
...
import Telomare (BasicExpr, UnsizedRecursionToken, pattern PairB, pattern ZeroB)

-- | The .tel core, plus the native recursion node (which replaces the
-- church tower telomare1's sizing would install).
data TelExpr
  = TZero
...
data Value
  ...
  | VAbort
  | VAborted BasicExpr
    -- ^ an abort VALUE: propagates and may be discarded (telomare1
    -- AbortedF); fatal only if it survives into the iteration result
...
data TelError
  = TelStuck String        -- ^ ill-formed application/projection (telomare1 `error`s)
...
-- | Abort payload truncation (telomare1 abortStep: Zero/Pair structural,
...
-- | Leftmost abort embedded anywhere in a result value (telomare1
-- checkError).  'VDefer' bodies are code, not values — nothing to scan.
...
evalTel expr env = case expr of
  ...
  -- the lazy if/then/else (telomare1 iteB_ shape); see module header
...
-- | Application dispatch (telomare1 FillFunction + abortStep rules).
...
-- .tel calling convention (what telomare1's appB/twiddle arranges).
